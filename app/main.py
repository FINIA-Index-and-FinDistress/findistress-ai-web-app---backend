...
app.add_middleware(
    ...
    allowed_hosts=[
        ...
        "127.0.0.1",
        "*.ngrok.io",
        os.getenv("ALLOWED_HOST", "https://findistress-ai-web-app-backend.onrender.com")  # Update default
    ]
)
...
def get_cors_origin(request: Request) -> str:
    """Return the matching CORS origin or raise HTTPException."""
    origin = request.headers.get("origin")
    if origin in FRONTEND_URLS:
        return origin
    logger.warning(f"Invalid origin: {origin}")
    return FRONTEND_URLS[0]  # Default to first allowed origin
